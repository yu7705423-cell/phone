...
import { Page, List, ListItem, Field, Segmented, Switch,
...
// 会话里的美化页只放「在这段会话里要动的」那几样：挂哪一份、导入、头像、消息时刻。
// 尺寸、自定义 CSS、导出、改名、删除都在「美化」app 里那一份的页面上 —— 那些是在改
// 这一份美化本身，改完挂着它的每段会话都跟着变，放在某一段会话里反而容易误会。
export function SkinPage({ chatId }) {
  ...
  const avatarGen = cur ? skin.gen.groupValues(cur.gen || {}, 'avatar') : {};
  const setShow = v => set({ gen: { ...(cur.gen || {}), avatar: { ...avatarGen, show: v } } });
  const openInSkin = route => phone.intent.open('skin', { route, back: true });

  const create = async () => {
    const name = await prompt({ title: '新建一份美化', placeholder: '名称', okText: '新建' });
    if (name === null) return;
    const row = skin.create({ name: name || '未命名' });
    skin.attach(chatId, row.id);
    openInSkin(`/gen/${row.id}`);
  };
  ...
  return html`
    <${Page} title="美化" onBack=${nav.pop}>
      ${cur ? html`<${Sample} char=${char} chat=${chat}/>` : html`
        <div class="settings-foot">
      ...
      ${cur ? html`
        <${List} title="这段会话">
          <${ListItem} title=${cur.name} subtitle="在「美化」中编辑尺寸、生成器与自定义 CSS" arrow multiline
            left=${html`<${Icon} name="sparkle" size=${18}/>`}
            onClick=${() => openInSkin(`/one/${cur.id}`)}/>
          ${skin.crashed(cur.id) ? html`
            <${ListItem} title="上次打开这段会话时没能正常显示" multiline
              subtitle="这一份已被暂停注入。在「美化」中改好之后可以恢复"
              onClick=${() => openInSkin(`/one/${cur.id}`)}/>` : null}
        ...
        <div class="pad-x pad-t">
          ...
          <${Field} label="显示头像"
            desc="不显示的那一边连位置一起收起，气泡贴着屏幕边缘。头像框、描边等更多样式在「美化」的生成器中。">
            <${Segmented} value=${avatarGen.show || 'both'} onChange=${setShow}
              items=${skin.gen.FACE_SHOW.map(x => ({ value: x.id, label: x.label }))}/>
          <//>
        </div>` : null}

      <${List} title=${cur ? '换一份' : '挂一份现成的'}>
        ${all.length ? all.map(x => html`
          <${ListItem} key=${x.id} title=${x.name}
            subtitle=${`已用于 ${skin.usedBy(x.id)} 段会话`}
            right=${cur && cur.id === x.id ? html`<${Icon} name="check" size=${16}/>` : null}
            onClick=${() => skin.attach(chatId, x.id)}/>`)
        : html`<${ListItem} title="还没有任何美化" subtitle="新建一份开始" multiline/>`}
      <//>
      <div class="pad btn-row">
        <${Button} variant="ghost" icon="plus" onClick=${create}>新建一份<//>
