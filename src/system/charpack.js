...
// 单个角色的打包与安装。
//
// 整库备份（backup.js）是「换设备、出事了恢复」，这一份是「把一个角色带走」：
// 发给别人、搬到另一台设备、或者在动大手术之前单独留一份。
//
// **两档**。分开是因为这两件事想要的东西正好相反：
//   card  只有角色卡本身（含小号、关联世界书、各种图）。给别人用。
//   full  再加上聊天记录与记忆。自己搬家用。
// 聊天记录是很私人的东西，发给别人的那一档默认不带，得自己选。
//
// 装回去时有三条规矩，都是为了「导入不该弄坏已经有的东西」：
//   1. 角色 id 撞了就**另建一个副本**，不覆盖。原来那个一个字都不动。
//   2. 世界书 id 撞了就**用本机现有那本**，不覆盖 —— 同一本世界书常被多个
//      角色关联，拿包里的盖掉，等于替另外几个角色改了设定。
//   3. 图片和音频按原 id 放回去，已经有的跳过。id 是时间加随机数，撞上
//      基本只发生在「同一个包导入两次」，那本来就是同一张图。
