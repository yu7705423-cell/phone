...
//
// 导出只有两种，别再分出第三种来：
//   完整备份（backup.js）  整个库。所有角色、曲库片库、外观，全都在里面
//   这个角色（本文件）      一个角色，以及与它相关的全部数据
//
// 所以这里**不分档**。「和这个角色相关的全部」就是全部：角色卡、它的小号、
// 它关联的世界书、与它的会话与消息、关于它的记忆，以及这些用到的图片和语音。
// history 这个参数留着是给测试和以后用的，产品里一律传 true。
//
