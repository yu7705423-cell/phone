...
处理办法不是改用 `dvh`，而是由 `shell/Root.js` 量出真实可视高度写进 `--app-h`，
**只在宽度变化（横竖屏切换）时重算**；地址栏显隐只改高度，一律忽略。
这样既不会被切掉，也不会像 `dvh` 那样随滚动抽动。

```css
.root { height: var(--app-h, 100vh); }   /* 100vh 只是兜底 */
```
...
一次性跑完五项检查：视口单位、零 emoji、导入导出、依赖边界、硬编码颜色。

其中「导入导出」是无构建方案必须自己补的一环：没有打包器替我们校验，
引用一个已经删掉的导出要到运行时打开那个页面才会炸。
