...
## 14. 注入 prompt 的文字，和界面文案同一个标准

**凡是会进 prompt 的句子，一律书面语、中性、祈使。** 骨架、各能力的说明、
各任务的 instruction、上下文区块里那几句提示，全都算。

这些是**说明书**，不是聊天。写的时候按写规格文档的腔调写，
不要带作者自己的语气和用词习惯。
...
```
错：这些是你自己的安排，不是任务清单。聊起来该提的时候提，别一上来就报一遍。
对：以上为你本人的安排，不是待办清单。聊到相关内容时再提及，不要在开始时复述全部安排。

错：拿不准的信息宁可丢掉，不要自己编
对：无法确认的信息予以舍去，不得自行补充
```

**称角色一律用「该角色」，不用「她」「他」。** 和第 7 条同一个理由：
性别由角色卡决定。prompt 里写「她」，模型会照着认下来。

举例、被禁止的原话用「」引起来，那里面本来就该是口语，不受此限。
输出格式那一段是 JSON，里面的直引号是语法。

由 `scripts/check-prompt-tone.mjs` 扫描拦截：只扫字符串字面量，
注释不管。它拦的是上面这几类**机械可查的**痕迹，
「这件事让你不痛快」这种它查不出来 —— 那一层仍然要自己把关。
