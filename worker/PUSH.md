...
**哪些手机收得到**：Web Push 只在浏览器和「添加到主屏幕」的网页里有。

| | 能不能用 |
|---|---|
| 安卓 Chrome / Edge（浏览器或添加到主屏幕） | 能 |
| iPhone：Safari 里「添加到主屏幕」之后，从主屏幕图标打开（iOS 16.4 及以上） | 能 |
| iPhone：直接在 Safari 标签页里用 | 不能（苹果只给主屏幕上的网页发推送） |
| 电脑 Chrome / Edge / Firefox | 能（浏览器要开着或在后台） |
| 安装版 apk、ipa | 不能（外壳里没有 Web Push），设置里不显示这一项 |
...
1. 用**安卓 Chrome**打开应用，或者 **iPhone 上从主屏幕图标**打开
...
| 看到的 | 原因与办法 |
|---|---|
| 设置里没有「后台消息」 | `site.js` 里 `pushServer` 没填，或者用的是 apk / ipa、Safari 标签页（见上面的表） |
