...
after = await state();
check(after.stack.length === before.stack.length - 1 && after.stack.slice(-1)[0] === '/',
  `右边缘左滑退了一级：${JSON.stringify(before.stack)} -> ${JSON.stringify(after.stack)}`);

// 11 右边缘往右滑（反方向）-> 不退
await go('chat', `/chat/${ids.chat}`);
await swipe(W - 5, 500, W - 2, 505);
after = await state();
check(after.stack.slice(-1)[0].startsWith('/chat/'), `右边缘反方向不退（${after.stack.slice(-1)[0]}）`);

// 12 屏幕中间往左滑 -> 不退
await go('chat', `/chat/${ids.chat}`);
await swipe(300, 500, 80, 505);
after = await state();
check(after.stack.slice(-1)[0].startsWith('/chat/'), `中间左滑不退（${after.stack.slice(-1)[0]}）`);

// 13 右边缘滑得不够远 -> 弹回去
await go('chat', `/chat/${ids.chat}`);
await swipe(W - 5, 500, W - 60, 500);
after = await state();
check(after.stack.slice(-1)[0].startsWith('/chat/'), `右边只滑 55px 不退（${after.stack.slice(-1)[0]}）`);
check(await page.evaluate(() => [...document.querySelectorAll('.page')].every(e => !e.style.transform)),
  '右边弹回去之后位移清干净了');

// 14 右边缘竖滑 -> 不退
await go('chat', `/chat/${ids.chat}`);
await swipe(W - 5, 700, W - 8, 300);
after = await state();
check(after.stack.slice(-1)[0].startsWith('/chat/'), `右边缘竖滑不退（${after.stack.slice(-1)[0]}）`);

// 15 右边缘滑的时候页面是朝左让开的（负位移），不是朝右
await go('chat', `/chat/${ids.chat}`);
await touch('touchStart', W - 5, 500);
for (let i = 1; i <= 6; i++) { await touch('touchMove', W - 5 - i * 20, 500); await page.waitForTimeout(12); }
const shift = await page.evaluate(() => {
  const el = [...document.querySelectorAll('.page')].find(e => e.style.transform);
  return el ? el.style.transform : '';
});
check(/translateX\(-\d/.test(shift), `右边缘拖动时页面朝左让开（${shift || '没有位移'}）`);
await touch('touchEnd', W - 125, 500);
await page.waitForTimeout(600);
...
await page.waitForTimeout(600);
check(await page.evaluate(() => /已选|全选/.test(document.body.innerText)), '又进了多选');
await swipe(W - 5, 500, 110, 505);
after = await state();
check(after.stack.slice(-1)[0].startsWith('/chat/'), `右边缘左滑没退路由（${after.stack.slice(-1)[0]}）`);
check(!await page.evaluate(() => /已选|全选/.test(document.body.innerText)),
  '右边缘左滑退出了多选（走的是这一页自己的返回）');
