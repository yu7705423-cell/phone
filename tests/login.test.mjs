...
await doLogin('小林', acct.password);
ok('密码对：进门', await booted());
...
// ---- 四 ----
await direct('/auth/login', { name: '小林', password: acct.password, device: 'other-1' });
await direct('/auth/login', { name: '小林', password: acct.password, device: 'other-2' });
await open();
...
await page.locator('.btn', { hasText: '新建账号' }).click();
await page.waitForTimeout(400);
await page.locator('.sheet').last().getByRole('textbox').first().fill('阿岚');
await page.locator('.sheet .btn', { hasText: '生成' }).click();
await page.waitForTimeout(1200);
let out = await page.locator('.acct-out').innerText().catch(() => '');
ok('新建一个：显示账号与密码', /账号：阿岚　密码：[A-Za-z2-9]{10}/.test(out), out);
const pwLan = /密码：(\S+)/.exec(out)?.[1];
const login2 = await direct('/auth/login', { name: '阿岚', password: pwLan, device: 'x' });
ok('生成的密码真的能登录', !!login2.token);
await page.locator('.sheet .btn', { hasText: '关闭' }).click();
await page.waitForTimeout(300);
await page.locator('.btn', { hasText: '批量新建' }).click();
await page.waitForTimeout(300);
await page.locator('.sheet').last().locator('input').first().fill('3');
await page.locator('.sheet .btn', { hasText: '生成' }).click();
for (let i = 0; i < 30 && !(await page.locator('.acct-out').count()); i++) await page.waitForTimeout(200);
out = await page.locator('.acct-out').innerText().catch(() => '');
ok('批量新建 3 个：三行账号与密码', out.split('\n').filter(l => /^账号：u\d{6}　密码：/.test(l)).length === 3, out);
