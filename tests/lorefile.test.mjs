...
ok('导出成一个 zip：下载了一个文件', downloads.length === 1 && /\.zip$/.test(downloads[0].suggestedFilename()),
...
ok('导出成单独的 docx：每本一个', names.length === 2 && names.every(n => /\.docx$/.test(n)), names.join(','));
const docxPath = join(OUT, 'lore.docx');
const one = downloads.find(d => /校园/.test(d.suggestedFilename()));
...
// ---- 三、四：别处来的文字 ----
const prose = join(OUT, '海边小镇.txt');
writeFileSync(prose, '小镇在海边，常年刮东南风。\n\n镇上只有一条街。');
const junk = join(OUT, '不要这本.txt');
...
await go('/');
await page.waitForTimeout(400);
await page.locator('input[type="file"]').setInputFiles([prose, junk]);
await page.waitForTimeout(1200);
const card = page.locator('.lb-draft', { hasText: '海边小镇' });
...
await page.locator('.lb-draft', { hasText: '不要这本' }).locator('.list-item', { hasText: '导入这一本' }).locator('.switch').click();
await page.waitForTimeout(200);
await page.locator('button', { hasText: /^保存 1 本$/ }).click();
await page.waitForTimeout(600);
const got = await books();
ok('关掉的那一本没有入库', got.length === 1 && got[0].name === '海边小镇', JSON.stringify(got));
