...
ok('打开一页，标题是谁的心声', /阿岚的心声/.test(sheet), sheet.slice(0, 80));
ok('上面是这一轮的心声和那一句', /这一轮/.test(sheet) && /第二轮的心声/.test(sheet) && /第二轮第二句/.test(sheet), sheet.slice(0, 200));
ok('下面是以前的那一条', /以前的 · 1/.test(sheet) && /第一轮的心声/.test(sheet), sheet.slice(0, 300));
...
await page.locator('.sheet-mask, .sheet-backdrop').first().click({ force: true }).catch(() => {});
