...
| 交给聊天模型 | `chat` | 不调识图接口。发起回复时，把最近几张用户图以 dataURL 挂在对应那条历史消息上 |
| 单独的识图接口 | `api` | `system/ai/vision.js` 先读成 `[图片：描述]` 写回 `content` |

`chat` 档由 `engine.js` 的 `imagesFor()` 取最近 `IMAGE_CARRY = 3` 张，
`buildHistory` 把它们放进 `{ role, content, image }`，再由各 provider 转成自己的
内容块形状（OpenAI 是 `image_url`，Anthropic 是 `source.type = 'base64'`）。
带图的两条消息不做相邻合并，合并会把图对应的那句话挪走。
只带最近三张：图比文字贵得多，整条历史都带上一次请求就能翻几倍。

三档都不把图片上传到任何中转存储，`chat` 档也只是把 dataURL 写进这一次请求体。
